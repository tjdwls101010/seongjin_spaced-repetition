...
export class SRSettingTab extends PluginSettingTab {
    ...
    private async tabFlashcards(containerEl: HTMLElement): Promise<void> {
        ...
        containerEl.createEl("h3", { text: "Section-Based Cards" });

        const sectionBasedCardsDesc = new Setting(containerEl)
            .setName("Enable section-based cards")
            .setDesc(
                "Creates ONE flashcard per note from specified markdown sections. Both sections must exist to create a card."
            );
        sectionBasedCardsDesc.addToggle((toggle) =>
            toggle
                .setValue(this.plugin.data.settings.enableSectionBasedCards)
                .onChange(async (value) => {
                    this.plugin.data.settings.enableSectionBasedCards = value;
                    await this.plugin.savePluginData();
                }),
        );

        new Setting(containerEl)
            .setName("Front heading")
            .setDesc("Markdown heading for the front of the card")
            .addText((text) =>
                text
                    .setValue(this.plugin.data.settings.sectionFrontHeading)
                    .setPlaceholder("# 📋정리")
                    .onChange((value) => {
                        applySettingsUpdate(async () => {
                            this.plugin.data.settings.sectionFrontHeading = value;
                            await this.plugin.savePluginData();
                        });
                    }),
            )
            .addExtraButton((button) => {
                button
                    ...
                    .setTooltip(t("RESET_DEFAULT"))
                    .onClick(async () => {
                        this.plugin.data.settings.sectionFrontHeading =
                            DEFAULT_SETTINGS.sectionFrontHeading;
                        await this.plugin.savePluginData();
        ...
        new Setting(containerEl)
            .setName("Back heading")
            .setDesc("Markdown heading for the back of the card")
            .addText((text) =>
                text
                    .setValue(this.plugin.data.settings.sectionBackHeading)
                    .setPlaceholder("# 💭관련")
                    .onChange((value) => {
                        applySettingsUpdate(async () => {
                            this.plugin.data.settings.sectionBackHeading = value;
                            await this.plugin.savePluginData();
                        });
                    }),
            )
            .addExtraButton((button) => {
                button
                    ...
                    .setTooltip(t("RESET_DEFAULT"))
                    .onClick(async () => {
                        this.plugin.data.settings.sectionBackHeading =
                            DEFAULT_SETTINGS.sectionBackHeading;
                        await this.plugin.savePluginData();
