...
export class Question {
    ...
    formatScheduleAsHtmlComment(settings: SRSettings): string {
        // For section-based cards, use different comment marker
        const commentBegin = this.questionType === CardType.SectionBased
            ? SR_SECTION_COMMENT_BEGIN
            : SR_HTML_COMMENT_BEGIN;

        let result: string = commentBegin;
    ...
    formatForNote(settings: SRSettings): string {
        // Section-based cards don't include scheduling inline - it goes at end of note
        if (this.questionType === CardType.SectionBased) {
    ...
    updateQuestionText(noteText: string, settings: SRSettings): string {
        // Section-based cards handle scheduling at end of note
        if (this.questionType === CardType.SectionBased) {
            return this.updateSectionBasedQuestionText(noteText, settings);
        }
    ...
    private updateSectionBasedQuestionText(noteText: string, settings: SRSettings): string {
        // For section-based cards, we update the scheduling at the end of the note
        // First, remove any existing section-based scheduling
        let newText = NoteCardScheduleParser.removeSectionScheduleInfo(noteText);

        // Check if we need to add scheduling (only if saving on note file and has schedule)
        if (settings.dataLocation === DataLocation.SaveOnNoteFile) {
            const hasSchedule: boolean = this.cards.some((card) => card.hasSchedule);
            if (hasSchedule) {
                const scheduleHtml = this.formatScheduleAsHtmlComment(settings);
                // Ensure there's a newline before the scheduling comment
                if (!newText.endsWith("\n")) {
                    newText += "\n";
                }
                newText += scheduleHtml + "\n";
            }
        }
