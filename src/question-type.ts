...
class QuestionTypeSectionBased implements IQuestionTypeHandler {
    expand(questionText: string, settings: SRSettings): CardFrontBack[] {
        // questionText contains: front section + "\n---SECTION-BREAK---\n" + back section
        const parts = questionText.split("\n---SECTION-BREAK---\n");
        if (parts.length !== 2) {
            console.error("QuestionTypeSectionBased: Invalid section-based card format");
            return [];
        }
    ...
    static create(questionType: CardType): IQuestionTypeHandler {
        ...
        switch (questionType) {
            ...
            case CardType.Cloze:
                ...
                break;
            case CardType.SectionBased:
                handler = new QuestionTypeSectionBased();
                break;
        }
        return handler;
    }
