...
export class NoteCardScheduleParser {
    static createCardScheduleInfoList(questionText: string, isSectionBased: boolean = false): CardScheduleInfo[] {
    ...
    static createCardScheduleInfoListFromNoteText(noteText: string): CardScheduleInfo[] {
        // For section-based cards, look for <!--SR-SECTION:...-->  at the end of the note
        const sectionScheduleRegex = /<!--SR-SECTION:(.*?)-->/g;
        let scheduling: RegExpMatchArray[] = [...noteText.matchAll(sectionScheduleRegex)];

        if (scheduling.length === 0) {
            return [];
        }

        // Extract the schedule info from the SR-SECTION comment
        const scheduleText = scheduling[0][1];
        const scheduleMatches: RegExpMatchArray[] = [...scheduleText.matchAll(MULTI_SCHEDULING_EXTRACTOR)];

        return this.createInfoList(scheduleMatches);
    }
    ...
    static removeSectionScheduleInfo(noteText: string): string {
        return noteText.replace(/<!--SR-SECTION:.+-->\n?/gm, "");
    }
}
